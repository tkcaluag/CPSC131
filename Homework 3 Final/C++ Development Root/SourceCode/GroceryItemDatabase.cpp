...
GroceryItemDatabase::GroceryItemDatabase( const std::string & filename )
{
  ...
  while( fin >> workingItem ) {
    _items.push_back( std::move(workingItem) );
  }
...
///////////////////////// TO-DO (3) //////////////////////////////
  ...
  GroceryItem * GroceryItemDatabase::find( std::size_t index, const std::string & upc ) {
    if( _items.size() == index) {
      return nullptr;
        }

    if( _items.at(index).upcCode() == upc ) return &_items.at(index);
