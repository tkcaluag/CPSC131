...
void GroceryList::insert( const GroceryItem & groceryItem, std::size_t offsetFromTop )        // insert provided grocery item at offsetFromTop, which places it before the current grocery item at offsetFromTop
{
  ...
  { /**********  Part 1 - Insert into array  ***********************/
    ...
    if( _gList_array_size >= _gList_array.size() ) throw CapacityExceeded_Ex( "Capacity Exceeded" );

    auto pos = _gList_array.begin() + offsetFromTop;

    std::shift_right( pos, _gList_array.end(), 1 );
  ...
  { /**********  Part 2 - Insert into vector  **********************/
    ...
    auto position = _gList_vector.begin();
    position      = std::next( position, offsetFromTop );

    _gList_vector.insert( position, groceryItem );
  ...
  { /**********  Part 3 - Insert into doubly linked list  **********/
    ...
    auto position = _gList_dll.begin();
    position      = std::next( position, offsetFromTop );
    _gList_dll.insert( position, groceryItem );
  ...
  { /**********  Part 4 - Insert into singly linked list  **********/
    ...
    auto position = _gList_sll.before_begin();
    position      = std::next( position, offsetFromTop );
    _gList_sll.insert_after( position, groceryItem );
...
void GroceryList::remove( std::size_t offsetFromTop )
{
  ...
  { /**********  Part 1 - Remove from array  ***********************/
    ...
    auto position = _gList_array.begin() + offsetFromTop;
    std::shift_left( position, _gList_array.end(), 1 );
...
void GroceryList::moveToTop( const GroceryItem & groceryItem )
{
  ...
  if( auto index = find( groceryItem ); index != _gList_array_size )
  {
    remove( index );
...
GroceryList & GroceryList::operator+=( const GroceryList & rhs )
{
  ...
  for( auto const & item : rhs._gList_vector )
  {
    insert( item, Position::BOTTOM );
  }
...
std::weak_ordering GroceryList::operator<=>( GroceryList const & rhs ) const
{
  ...
  for( std::size_t index = 0;  index < commonExtent; ++index){
    if( auto result = _gList_array[index] <=> rhs._gList_array[index]; result != 0){
      return result;
    }
  }
...
bool GroceryList::operator==( GroceryList const & rhs ) const
{
  ...
  for( std::size_t index = 0; index < _gList_array_size; ++index )
  {
    if( rhs._gList_array[index] != _gList_array[index] )
    {
      return false;
    }
  }
...
std::istream & operator>>( std::istream & stream, GroceryList & groceryList )
{
  ...
  while( stream >> workingItem )
  {
    groceryList.insert( workingItem, groceryList._gList_array_size );
  }
