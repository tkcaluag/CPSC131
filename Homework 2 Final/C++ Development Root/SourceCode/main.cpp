...
int main()
{
  try
  {
    ...
    GroceryList partsList = { { "CPU", "Intel" }, { "Motherboard", "MSI" }, { "GPU", "NVIDIA" }, { "OS", "Windows" } };

    std::cout << '\n' << "My PC Parts List:" << partsList << '\n';

    //Forgot to add the storage and power supply.
    partsList += { { "SSD", "Samsung" }, { "HDD", "Seagate" }, { "Power Supply", "Corsair" } };

    //Need some accessories

    GroceryList otherList = { { "Keyboard", "Razer" }, { "Headset", "SteelSeries" }, { "Mouse", "Logitech" } };

    //I need a new mouse more than anything so...
    otherList.moveToTop( { "Mouse", "Logitech" } );

    std::cout << '\n' << "Accessories List:" << otherList << '\n';

    //I changed my mind, I dont want an Intel CPU. Instead I want an AMD one.

    partsList.remove( { "CPU", "Intel" } );
    partsList.insert( { "CPU", "AMD" }, GroceryList::Position::TOP );

    //Oops I also forgot about the RAM.

    partsList.insert( { "16 GB RAM", "Corsair" }, partsList.find( { "OS", "Windows" } ) );

    //I should probably include the PC Case as well
    partsList.insert( { "Case", "NZXT" }, GroceryList::Position::BOTTOM );

    std::cout << '\n' << "Updated PC Parts List:" << partsList << '\n';

    //I want to compare my friend's accessory list to mine and add whatever I don't have.
    //I'll ask him to give me his list so I can compare the two.

    std::istringstream friendInput( R"( "",   "Razer",             "Keyboard",        0.0
                                        "",   "Logitech",             "Mouse",        0.0
                                        "",   "",             "144hz Monitor",        0.0
                                        "",   "",              "60hz Monitor",        0.0
                                        "",   "",                  "Mousepad",        0.0
                                        "",   "Razer",           "Microphone",        0.0 )" );
    GroceryList        friendList;
    friendInput >> friendList;

    std::cout << '\n' << "My Friend's Accessories List:" << friendList << '\n';

    //I'm now going to add whatever my friend has that I dont into my list
    if( friendList != otherList ){
      otherList += friendList;
      std::cout << '\n' << "Combined Accessories List:" << otherList << '\n';
    }

    //Now I want to compare my PC parts list with my friends.

    const GroceryList friendPartsList = { { "CPU", "Intel" },
                                    { "Motherboard", "ASUS" },
                                    { "GPU", "AMD" },
                                    { "32 GB RAM", "G.Skill"},
                                    { "OS", "Windows" },
                                    { "SSD", "Crucial" },
                                    { "HDD", "Seagate"},
                                    { "Power Supply", "Gigabyte"},
                                    { "Case", "Cooler Master"} };

    std::cout << '\n' << R"(Which parts list is the "greater" or "lesser"?)" << '\n';

    if( auto result = friendPartsList <=> partsList; result != 0 )
    {
      if( result < 0 ){
        std::cout << '\n'
                  << "My Friend's List is Greater"
                  << friendPartsList << '\n';
      } else {
        std::cout << '\n'
                  << "My List is Greater:"
