...
GroceryStore::GroceryStore( const std::string & persistentInventoryDB )
{
  ...
  while( fin >> std::quoted( code ) >> quantity ){
    _inventoryDB[code] = quantity;
  }
...
GroceryStore::GroceryItemsSold GroceryStore::ringUpCustomer( const ShoppingCart & shoppingCart, std::ostream & receipt )
{
  ...
  for( auto item : shoppingCart ){
    ...
    std::string upcCode = item.first;
    GroceryItem * workingItem = worldWideGroceryDatabase.find( upcCode );

    if( workingItem == nullptr ){
      receipt << '"' << upcCode << "\" (" << item.second.productName() << ") not found, the item is free!" << '\n';
    } else {
      receipt << *workingItem << '\n';
      amountDue += workingItem->price();
      if( _inventoryDB.find( upcCode ) != _inventoryDB.end() ) {
...
void GroceryStore::reorderItems( GroceryItemsSold & todaysSales, std::ostream & reorderReport )
{
  ...
  ///////////////////////// TO-DO (5) //////////////////////////////
    ...
    for( const auto &code : todaysSales ){

      if( (_inventoryDB.find( code ) == _inventoryDB.end()) || ( _inventoryDB[code] < REORDER_THRESHOLD ) ){
