...
GroceryStore::GroceryStore( const std::string & persistentInventoryDB )
{
  ...
  if( fin >> std::ws >> std::quoted( item.first ) && fin >> std::ws >> item.second ){
    _inventoryDB.insert( item );
  }
...
GroceryStore::GroceryItemsSold GroceryStore::ringUpCustomers( const ShoppingCarts & shoppingCarts, std::ostream & receipt )
{
  ...
  for( auto& tempCarts : shoppingCarts ){
    receipt << '\n'
            << tempCarts.first << "'s shopping cart contains:";
    todaysSales.merge( ringUpCustomer( tempCarts.second, receipt ) );
  }
...
GroceryStore::GroceryItemsSold GroceryStore::ringUpCustomer( const ShoppingCart & shoppingCart, std::ostream & receipt )
{
  ...
  unsigned int amountDue = 0;

  for( auto item : shoppingCart ){
    if( worldWideGroceryDatabase.find(item.second.upcCode()) == nullptr ){
      receipt << '"' << item.second.upcCode() << "\" (" << item.second.productName() << ") not found, the item is free!" << '\n';
    } else {
      receipt << item.second << '\n';
      amountDue += item.second.price();
      if( _inventoryDB.find( item.second.upcCode() ) != _inventoryDB.end() ) {
        _inventoryDB.at( item.second.upcCode() ) - 1;
        purchasedGroceries.insert( item.second.upcCode() );
      }
    }
    receipt << amountDue;
  }
...
void GroceryStore::reorderItems( GroceryItemsSold & todaysSales, std::ostream & reorderReport )
{
  ...
  ///////////////////////// TO-DO (5) //////////////////////////////
    ...
    for( auto code : todaysSales ){

      if( (_inventoryDB.find( code ) != _inventoryDB.end()) || ( _inventoryDB.size() < REORDER_THRESHOLD ) ){
        if( GroceryItem * item = worldWideGroceryDatabase.find( code ); item == nullptr )
        {
          reorderReport << code;
        }
        else
        ...
        if( auto itemStock = _inventoryDB.find( code ); itemStock == _inventoryDB.end() )
        {
          reorderReport << "*** no longer sold in this store and will not be reordered" << '\n';
        }
        else
        {
          reorderReport << "only " << itemStock->second
          << " remain in stock which is " << (REORDER_THRESHOLD - itemStock->second)
          << " unit(s) under the reorder threshold (15), re-ordering 20 more";
          _inventoryDB.at( code ) += LOT_COUNT;
        }
      }
    }
